...
def change_year(message):
    ...
    greet_message, year = get_or_save_user(message.chat.id, cursor, conn) # если есть чат с таким id, то возвращается id чата и текущий год, нет - то сначала сохраняется в бд
    global cur_year
    try:
...
def start_message(message):
    greet_message, year = get_or_save_user(message.chat.id, cursor, conn)
    if greet_message:
...
def ehco_message(message):
    ...
    if message.text == "Помощь":
        ...
        bot.send_message(message.chat.id, standings_check('drivers', year))

    elif message.text == 'Кубок конструктуров':
        bot.send_message(message.chat.id, standings_check('team', year))

    elif message.text == 'Список Гран-при':
        races_all = standings_check('races', year)
        ...
        bot.send_message(message.chat.id, standings_check('calendar', year=year))
